...
class TestSort{
	...
	public static void indexSelectionAssending(int[]arr,int start, int end){
	...
	public static int[] sortOddEven(int[]arr){
		...
		for (int i = 0; i < arr1.length; i++) {
			if(arr[i]%2==0){
				arr1[even--]= arr[i];
				indexSelection(arr, even, arr.length-1);
			}
			if(arr[i]%2!=0){
				arr1[odd++]= arr[i];
				indexSelection(arr, 0, odd);
			}
		}
		return arr1;
	}
